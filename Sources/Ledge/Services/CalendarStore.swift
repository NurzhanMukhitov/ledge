...
/// the one permission Ledge needs at all, and nobody should be asked for it
...
final class CalendarStore: ObservableObject {
    ...
    private static func hiddenCalendarIdentifiers() -> Set<String> {
        ...
        guard let disabled = stored as? [String: [String]] else {
            NSLog("Ledge: com.apple.iCal DisabledCalendars is no longer [String: [String]] — hidden calendars will be shown again")
            return []
        }
...
/// does (#36). A calendar Ledge has never been asked about takes the answer
