...
> Всё описанное ниже придумал и написал автор оригинала — здесь добавлен только
> конвертер файлов на полке. Если приложение вам пригодилось, звезду ставьте
> в первую очередь [ему](https://github.com/akalikbergenov/cyclop),
> и там же есть кнопка поддержки.
>
> **Что добавлено в форке (0.7.0, база — 0.6.5):**
>
> Правый клик по карточке на полке отдаёт файл в конвертер. Всё на родных
> фреймворках: ImageIO для картинок, PDFKit для страниц, AVFoundation для видео.
> Ни одной внешней зависимости, ставить получателю нечего.
>
> - **Картинки** — в JPEG, сжатие тремя ступенями, картинки в PDF, удаление
>   геометок и даты съёмки. Веса ступеней измеренные: файл кодируется трижды и
>   показывает, что вышло, вместо ползунка качества, по которому ничего не понять.
> - **Видео** — mov в mp4 переупаковкой без перекодирования (секунды вместо минут),
>   сжатие тремя ступенями с уменьшением до 1920, извлечение звука в m4a и mp3.
>   Веса ступеней тут посчитанные, а не измеренные, и помечены знаком `≈`.
> - **MP3** — macOS его писать не умеет, поэтому в бандле едет LAME: 579 КБ,
>   arm64 + x86_64, грузится через `dlopen`. Динамически, а не статически —
>   именно это делает LGPL совместимым с MIT. Текст лицензии лежит рядом
>   с библиотекой, в `Contents/Frameworks`.
> - **Переводчик** перестал упираться в «пакет не установлен», когда пакет
>   установлен: `Locale.Language("en")` разворачивается в `en-US`, а скачан
>   бывает `en-GB`. Заодно языки теперь выбираются — заголовок колонки стал меню.
> - Долгая работа показывает процент прямо на карточке и отменяется крестиком.
