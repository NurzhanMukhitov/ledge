...
> **Форк [akalikbergenov/cyclop](https://github.com/akalikbergenov/cyclop).**
> Само приложение — челку как рабочую панель, плеер, полку, буфер, календарь,
> переводчик, телесуфлер — придумал и написал он. Мы взяли его работу и докрутили
> под свои задачи: нам нужно было жать и конвертировать файлы, не выходя из панели.
> Если приложение вам пригодилось, загляните [к автору](https://github.com/akalikbergenov/cyclop)
> — звезда и поддержка полагаются в первую очередь ему.

## Установка за минуту

1. Скачать **[Ledge-0.8.0.dmg](https://github.com/NurzhanMukhitov/ledge/releases/latest)**
2. Открыть образ и перетащить `Ledge` в «Программы»
3. Первый запуск macOS заблокирует — это нормально, приложение не подписано
   платным сертификатом Apple. Разрешить один раз:
   **Системные настройки → Конфиденциальность и безопасность → «Все равно открыть»**

Кому привычнее Терминал, третий шаг заменяется одной строкой — но **сначала
перетащите приложение в «Программы»**, иначе команде нечего снимать:

```bash
xattr -cr /Applications/Ledge.app
```

Она **ничего не печатает** — это и есть успех. Приложение она не запускает:
после нее откройте `Ledge` обычным двойным щелчком.

Дальше приложение живет в челке: наведите мышь на вырез сверху экрана.
Подробности и обновление — в разделе [Установка](#установка),
удаление — в разделе [Как удалить](#как-удалить).

## Что мы добавили

Правый клик по карточке на полке отдает файл конвертеру. Все на родных
фреймворках — ImageIO, PDFKit, AVFoundation, — так что получателю сборки
устанавливать нечего.

- **Картинки.** В JPEG, сжатие тремя ступенями, картинки в PDF, удаление
  геометок и даты съемки. Веса ступеней измеренные: файл кодируется трижды и
  показывает, что вышло, вместо ползунка качества, по которому ничего не понять.
- **Видео.** mov в mp4 переупаковкой без перекодирования — 477 МБ за 2,9 секунды
  вместо минут. Сжатие тремя ступенями с уменьшением до 1920: 28-минутная запись
  экрана ужимается с 3,5 ГБ до 662 МБ. Звук вынимается в m4a и mp3.
- **Форматы, которые macOS не открывает.** WMV, MKV, FLV, WebM — система не
  читает их вовсе. В бандле едет минимальный ffmpeg под LGPL (9,9 МБ, arm64 +
  x86_64, собран с `--disable-gpl`), он их конвертирует в MP4, M4A и MP3.
  Запускается отдельным процессом, а не линкуется, — при таком раскладе LGPL и
  MIT не спорят вовсе. Ставить получателю по-прежнему нечего.
- **MP3.** macOS его писать не умеет, поэтому в бандле едет LAME: 579 КБ,
  arm64 + x86_64, грузится через `dlopen`. Динамически, а не статически — именно
  это делает LGPL совместимым с MIT. Текст лицензии лежит рядом с библиотекой.
- **Переводчик.** Перестал упираться в «пакет не установлен», когда пакет
  установлен: `Locale.Language("en")` разворачивается в `en-US`, а скачан бывает
  `en-GB`. Заодно языки теперь выбираются — заголовок колонки стал меню.
- **Долгая работа видна.** Карточка появляется сразу, показывает процент и
  отменяется крестиком.
...
![Панель Ledge](docs/panel.png)

**[Скачать последнюю версию](https://github.com/NurzhanMukhitov/ledge/releases/latest)** —
...
Трек на снимке играет во вкладке браузера — Ledge читает его из самой macOS, не
...
| **Заготовки** | Ручной список того, что надоело набирать: почта, телефон, адрес. Добавляются кнопкой прямо в панели, удаляются крестиком на карточке; клик кладёт строку в буфер обмена. Тот же список лежит файлом в `~/Library/Application Support/Ledge/snippets.json` и правится в редакторе, если так удобнее |
...
Открыть `Ledge-<версия>.dmg` и перетащить приложение в «Программы».
...
строка про Ledge и кнопка **«Все равно открыть»**.
...
Кладет `build/Ledge-<версия>.dmg` — приложение и ярлык `/Applications` рядом.
...
Готовые образы лежат на [странице релизов](https://github.com/NurzhanMukhitov/ledge/releases) —
это и есть ссылка, которую можно давать людям вместо файла.

## Как удалить

Три уровня, от «убрать с глаз» до «стереть все следы». Выбирайте нужный.

**1. Просто убрать приложение.** Перетащите `Ledge` из «Программ» в корзину.
Настройки и файлы останутся — поставите заново, все будет как было.

**2. Убрать вместе с настройками.** То же плюс:
...
Здесь лежат сниппеты, заметки, сценарий телесуфлера и все переключатели:
что показано на полке, выбранная пара языков, режим приватности.

**3. Стереть все.** Дополнительно к предыдущему:

```bash
rm -rf ~/Pictures/Ledge
```

⚠️ **Здесь ваши файлы**, а не служебные данные: сохраненные из буфера скриншоты и
все, что вы конвертировали. Загляните туда до удаления — приложение эту папку
никогда само не чистит именно поэтому.

**Автозапуск** снимать отдельно не нужно: он живет в системном реестре, и macOS
убирает запись, когда приложение исчезает. Если хочется убедиться —
Системные настройки → Основные → Объекты входа.

**Доступ к календарю**, если вы его давали, снимается там же:
...
Доступ к Календарю — единственное разрешение, которое Ledge вообще запрашивает.
...
`~/Library/Application Support/Ledge/snippets.json`, `label` необязателен. Пункт
...
Пишется всё в `~/Library/Application Support/Ledge/notes.json` через долю
...
`Sources/LedgeMediaHelper/helper.m` компилируется в `libledgemedia.dylib`,
...
- Полка ссылается на файлы, а не копирует их: если переместить оригинал, карточка
  ...
  они сохраняются в `~/Pictures/Ledge` и не удаляются автоматически никогда,
